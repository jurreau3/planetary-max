...
export async function callKernel(env: Bindings, envelope: KernelEnvelope): Promise<Response> {
  ...
  return kernel.fetch(new Request(KERNEL_BRIDGE_URL, {
    method: 'POST',
...
export function resultResponse(result: KernelResult, upstreamStatus: number): Response {
  const status = result.ok
    ? upstreamStatus >= 200 && upstreamStatus < 300 ? upstreamStatus : 200
...
function failureResult(code: string, message: string, envelope?: KernelEnvelope): KernelResult {
  return {
    ok: false,
    error: { code, message },
    ...(envelope ? { meta: { messageId: envelope.id, type: envelope.type } } : {}),
  };
}
...
function defaultGovernance(mode: UmbrellaMode) {
  return { mode, decision: mode === 'off' ? 'bypassed' : mode === 'advisory' ? 'advisory' : 'allowed', deltas: [] } as const;
}
